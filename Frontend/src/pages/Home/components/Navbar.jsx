    ...
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white shadow-sm">
      <nav className="mx-auto flex w-full items-center justify-between px-4 py-3 lg:px-8">
        ...
        <div className="hidden shrink-0 items-center gap-3 lg:flex lg:gap-4">
          {user?.role === 'admin' ? (
            <Link to="/admin/properties" className="rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 transition duration-200 hover:bg-slate-50">
              Admin Panel
            </Link>
          ) : null}
          <Link to="/post-property" className="group relative overflow-hidden rounded-lg bg-(--color-cta-orange) px-5 py-2.5 text-sm font-semibold text-white shadow-lg transition duration-200 hover:-translate-y-0.5 hover:shadow-xl hover:brightness-95">
          ...
          {isAuthenticated ? (
            <div className="relative" ref={accountMenuRef}>
              <button
                ...
                className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm transition duration-200 hover:border-slate-300 hover:bg-slate-50"
              >
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-(--color-secondary-bg) text-(--color-primary)">
                  <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5.121 17.804A10.96 10.96 0 0112 15.5c2.62 0 5.027.92 6.879 2.454M15 11a3 3 0 11-6 0 3 3 0 016 0zm7 1a10 10 0 11-20 0 10 10 0 0120 0z" />
                  </svg>
                </div>
                <div className="text-left leading-tight">
                  <p className="text-sm text-slate-800">{displayName}</p>
                  <p className="text-xs text-(--color-primary)">{roleLabel}</p>
                </div>
              ...
              {isAccountMenuOpen ? (
                <div className="absolute right-0 top-full z-50 mt-2 w-80 rounded-2xl border border-slate-100 bg-white p-2 shadow-2xl">
                  <div className="border-b border-slate-100 px-3 py-3">
                    <p className="text-sm font-semibold text-slate-900">Welcome, {displayName}</p>
                    <p className="text-xs text-slate-500">Manage all your property actions from here</p>
                  </div>
                  ...
                  <div className="py-2">
                    {accountMenuItems.map((item) => (
                      <Link
                        ...
                        className="block rounded-xl px-3 py-2 transition duration-150 hover:bg-slate-50"
                      >
                  ...
                  <div className="border-t border-slate-100 px-2 pt-2">
                    <button
                      ...
                      className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 transition duration-200 hover:bg-slate-100"
                    >
                      Logout
                    </button>
                  </div>
            ...
            <Link to="/auth" className="relative flex items-center gap-2 transform rounded-lg border-2 border-(--color-primary) px-5 py-2 text-sm font-semibold text-(--color-primary) transition duration-200 hover:translate-x-0.5 hover:bg-(--color-secondary-bg)">
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {isOpen && (
        <div className="border-t border-slate-100 bg-white px-4 py-4 shadow-sm lg:hidden">
          ...
          <div className="mt-5 flex flex-col gap-3">
            {user?.role === 'admin' ? (
              <Link to="/admin/properties" className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-center text-sm font-semibold text-slate-700 transition duration-200 hover:bg-slate-50">
                Admin Panel
              </Link>
            ) : null}
            <Link to="/post-property" className="group relative flex items-center gap-2 overflow-hidden rounded-lg bg-(--color-cta-orange) px-4 py-2 text-sm font-semibold text-white shadow-md transition duration-200 hover:brightness-95">
              <span className="absolute inset-0 bg-linear-to-r from-transparent via-white to-transparent opacity-0 transition duration-300 group-hover:opacity-20" />
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              </svg>
            ...
            {isAuthenticated ? (
              <>
                ...
                <div className="rounded-lg border border-slate-200 bg-white p-3">
                  ...
                  <div className="mt-2 space-y-2">
                    {accountMenuItems.map((item) => (
                      <Link
                        key={item.label}
                        to={item.to}
                        className="block rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
                      >
                        {item.label}
                      </Link>
                    ))}
                  ...
                  <button
                    type="button"
                    onClick={logout}
                    className="mt-3 w-full rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition duration-200 hover:bg-slate-100"
                  >
                    Logout
                  </button>
                </div>
              ...
              <Link to="/auth" className="relative flex items-center gap-2 rounded-lg border-2 border-(--color-primary) px-4 py-2 text-sm font-semibold text-(--color-primary) transition duration-200 hover:bg-(--color-secondary-bg)">
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
                Login / Signup
